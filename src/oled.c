 ...
 * @brief 清空要移动物体的上一帧
 */
void OLED_ClearCharacter(uint8_t x, uint8_t y, char character, uint8_t width, uint8_t height) {
    // 创建一个数据缓冲区，用于存储字符的空白（全黑）数据
    uint8_t clearData[width * height / 8];
    memset(clearData, 0xFF, sizeof(clearData)); // 填充为全黑（背景色），以便清除字符

    // 调用 OLED_SetBlock 函数，用全黑的数据覆盖字符的区域
    OLED_SetBlock(x, y, clearData, width, height,0);
}
...
void OLED_PrintNumber(uint8_t x, uint8_t y, int number, const Font *font, OLED_ColorMode color)
{
    char buffer[12];  // 足够存储一个 32 位整数的字符串形式（最多11个字符 + '\0'）
    
    // 将整数转换为字符串
    sprintf(buffer, "%d", number);
    
    // 调用 OLED_PrintString 来显示数字字符串
    OLED_PrintString(x, y, buffer, font, color);
}
 ...
 * @param y 起始点纵坐标
 * @param nummber 数字
 * @param howmany 指定小数位数
 * @param font 字体
 * @param color 颜色
 **/
/*
void OLED_PrintFloat(uint8_t x, uint8_t y, float number, const Font *font, OLED_ColorMode color)
{
    char buffer[4];  // 足够存储一个浮点数的字符串形式
    
    // 使用 sprintf 将浮点数转换为字符串，保留指定的小数位
    sprintf(buffer, "%.2f", number);
    
    // 调用 OLED_PrintString 来显示浮点数字符串
    OLED_PrintString(x, y, buffer, font, color);
}
*/